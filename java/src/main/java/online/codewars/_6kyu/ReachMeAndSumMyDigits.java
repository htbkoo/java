 ...
 * ---
 * <p>
 * https://www.codewars.com/kata/reach-me-and-sum-my-digits/train/java
 */
...
public class ReachMeAndSumMyDigits {
    public static class SumDigNth {

        public static long sumDigNthTerm(long initval, long[] patternl, int nthterm) {
            // your code
            Function<Long, Long> getSumDigit = l -> String.valueOf(l).chars().mapToLong(i -> i - '0').sum();
            Function<long[], Long> getArraySum = is -> Arrays.stream(is).sum();

            final long sumOfPattern = getArraySum.apply(patternl);
            final int length = patternl.length;
            final int appliedTimes = nthterm - 1;
            final int numCycle = appliedTimes / length;
            final int remainNum = appliedTimes % length;
            return getSumDigit.apply(initval + sumOfPattern * numCycle + getArraySum.apply(Arrays.copyOf(patternl, remainNum)));
        }
    }
}