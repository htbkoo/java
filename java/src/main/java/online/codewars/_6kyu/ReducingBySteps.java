...
//            Yes this is the Euclid's algorithm from wikipedia
...
//            ugly hackish way - without creating my own collector
            final long[] previousElement = new long[1];
            return IntStream.
                    range(0, arr.length).
                    mapToLong(i -> previousElement[0] = operator.applyAsLong((i == 0 ? init : previousElement[0]), arr[i])).
                    toArray();
        }

        @SuppressWarnings("unused")
        public static long[] operArray2(BiFunction<Long, Long, Long> operator, long[] arr, long init) {
            // your code and complete ... before operator
//            ugly hackish way - without creating my own collector
            final long[] previousElement = new long[1];
            return IntStream.
                    range(0, arr.length).
                    mapToLong(i -> previousElement[0] = operator.apply((i == 0 ? init : previousElement[0]), arr[i])).
                    toArray();
        }
    }
}
