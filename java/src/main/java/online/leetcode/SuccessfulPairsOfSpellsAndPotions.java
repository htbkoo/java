...
https://leetcode.com/classic/problems/successful-pairs-of-spells-and-potions/description/?envType=daily-question&envId=2025-10-08
...
class SuccessfulPairsOfSpellsAndPotions {
    public int[] successfulPairs(int[] spells, int[] potions, long success) {
        potions = Arrays.copyOf(potions, potions.length); // make a copy to avoid mutating
        Arrays.sort(potions);

        var answer = new int[spells.length];
        for (var i = 0; i < spells.length; ++i) {
            answer[i] = getNumSuccessPairs(spells[i], potions, success);
        }
        return answer;
    }

    private int getNumSuccessPairs(int spell, int[] potions, long success) {
