    ...
    static class Solution {
        public boolean canCross(int[] stones) {
            if (stones == null || stones.length < 2) {
                ...
                return !jumps.get(n - 1).isEmpty();
            }
        }

        private boolean canJump(Set<Integer> jump, int distance) {
            return jump.contains(distance - 1) || jump.contains(distance) || jump.contains(distance + 1);
