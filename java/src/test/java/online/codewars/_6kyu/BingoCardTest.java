...
public class BingoCardTest {
    ...
    public void eachNumberOnCardIsUnique() {
        for (int i = 0; i < 10; i++) {
            ...
            assertTrue(String.format("Card numbers are not unique: %s", String.join(" ", (CharSequence[]) card)), set.size() == card.length);
        }
    }
    ...
    private void checkColumn(String[] card, @SuppressWarnings("UnusedParameters") String column, int start, int end, int beginRange, int endRange) {
        for (int i = start - 1; i < end; i++) {
    ...
    private int checkColumnOnRandomness(String[] card, @SuppressWarnings("UnusedParameters") String column, int start, int end) {
        int n = 0;
        boolean up = false;
        boolean down = false;
